...
description: |
  Python 包管理、依赖解析、虚拟环境和锁文件的指南。
  适用于：安装包、管理依赖、在 pip/uv/poetry/pdm/conda 之间选择、虚拟环境设置、锁文件策略、私有包索引、用于 CLI 工具的 pipx
  不适用于：配置 pyproject.toml 或构建后端（使用 project-system）、构建 CLI 应用（使用 cli）
license: MIT
metadata:
  displayName: "Python 包管理"
